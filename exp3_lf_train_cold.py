...
baseModelDir = create_path(f"{PARAMS.saved_models_path}EXP03_COLD/")


"""NETWORK TRAINING"""

with open(f"{csvDir}/EXP03_LF_COLD.csv", 'w', newline='') as file:
    writer = csv.writer(file)
    writer.writerow(["Method", "Iteration", "R@1 Cloudy", "R@1 Night", "R@1 Sunny", "R@1 Avg."])

    input_types = ["RGB", "GRAYSCALE", "HUE", "MAGNITUDE", "ANGLE"]
    
    print("Training Late Fusion COLD database")

    for input_type in input_types:

        vmDataset = datasets_cold.Database_wo_Fusion(input_type=input_type, tf=tf)
        ...
        trainDataset = datasets_cold.Train_LF(input_type=input_type, tf=tf)
            ...
            if i % int(len(trainDataloader) / PARAMS.num_validations) == 0 and i > 0:
                ...
                with torch.no_grad():
                    ...
                    for ilum in condIlum:
                        ...
                        testDataset = datasets_cold.Test_wo_Fusion(il=ilum, input_type=input_type, tf=tf)
                        testDataloader = DataLoader(testDataset, num_workers=0, batch_size=1, shuffle=False)

                        pkl_path = f"PKL_FILES/no_fusion/{input_type}/FR_A_{ilum}_val.pkl"
