...
with open(f"{csvDir}/EF_360Loc.csv", 'w', newline='') as file:
    ...
    for method in ef_methods:
        ...
        vmDataset = datasets_360loc.Database_EF(ef_method=method, input_type=input_type, tf=tf, env="atrium", il="daytime_360_0")
        ...
        trainDataset = datasets_360loc.Train_EF(ef_method=method, tf=tf, input_type=input_type)
        trainDataloader = DataLoader(trainDataset, shuffle=False, num_workers=0, batch_size=PARAMS.batch_size)

        netDir = create_path(os.path.join(baseModelDir, method))
        ...
        for ilum in condIlum:
            testDataset = datasets_360loc.Test_EF(il=ilum, env="atrium", ef_method=method, input_type=input_type, tf=tf)
            ...
            if i % int(len(trainDataloader) / PARAMS.num_validations) == 0 and i > 0:
                ...
                with torch.no_grad():
                    ...
                    for ilum in condIlum:
                        ...
                        testDataset = datasets_360loc.Test_EF(il=ilum, env="atrium", ef_method=method)
                        testDataloader = DataLoader(testDataset, num_workers=0, batch_size=1, shuffle=False)

                        pkl_path = f"PKL_FILES/EF/{method}/atrium_{ilum}_val.pkl"
