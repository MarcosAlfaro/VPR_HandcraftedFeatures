...
with open(csvDir + "EXP03_LF_360Loc.csv", 'a', newline='') as file:
    ...
    preprocessing_method = "MAGNITUDE"
    lf_methods = ["concat"]
    ...
    for lf in lf_methods:
        ...
        with torch.no_grad():
            ...
            for env in envs_360loc:
                ...
                vmDataset = datasets_360loc.Database_LF(env=env, input_type=preprocessing_method, enc="vitl", tf=tf, il=trainSeq)
                ...
                for ilum in condIlum:
                    ...
                    testDataset = datasets_360loc.Test_LF(il=ilum, env=env, input_type=preprocessing_method, enc="vitl", tf=tf)
